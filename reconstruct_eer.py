...
# ── Hook to capture 256-dim fusion embeddings ──────────────────────────────
emb_cache = []
...
# ── Step 1: build prototypes from train split ──────────────────────────────
...
# Sanity check — if EER > 20%, labels may be inverted, auto-flip and retry
if eer > 0.20:
    print("\n  WARNING: EER > 20% — trying inverted scores (label convention check)...")
    fpr2, tpr2, thr2 = roc_curve(dev_labels, -spoof_scores)
    ...
    if eer2 < eer:
        ...
        print("  --> Inversion didn't help. Model may need retraining or different scoring.")
else:
