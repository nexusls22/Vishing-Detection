...
def load_data(self) -> pd.DataFrame:
    ...
    for idx, filename in enumerate(selected_files):
        try:
            file_path = os.path.join(self.audio_path, filename + '.flac')
            ...
            row = protocol_df[protocol_df['file_name'] == filename].iloc[0]
            label = row['label']
            ...
            feature_dict = extract_audio_features(y, sr, filename)
...
# Extract features
def extract_audio_features(y: np.ndarray, sr: int, filename: str) -> dict:
...
## Put them together in one array, also merge new in load_data() and check for same shape
def extract_feature_array(df):
    # fao features
    fao = np.array([
        df['duration'].values,
...
# Check directory
def check_directory(path: str, description: str = "Directory") -> None:
    print(f"\n{description} ({path}):")
    print("-" * 40)
    try:
